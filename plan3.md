...
```bash
grcc \
  -o /home/zju/Desktop/oran-lab/radio/broker/build \
  /home/zju/Desktop/oran-lab/radio/broker/flows/multi_ue_3.grc
```
...
```bash
/usr/bin/python3 /home/zju/Desktop/oran-lab/radio/broker/build/multi_ue_scenario.py \
...
本机的 iperf3 server 一次只接受一个测试，因此并发测试使用三个 server ports：
...
sudo ip netns exec ue2 iperf3 -c 10.45.0.1 -p 5202 -u -b 2M -t 30 &
sudo ip netns exec ue3 iperf3 -c 10.45.0.1 -p 5203 -u -b 2M -t 30 &
wait
```
...
- [x] 官方 `multi_ue_scenario.grc` 已保留在 `upstream`。
- [x] OCUDU 多 UE config 没有覆盖单 UE config。
- [x] 三份 srsUE config 的 IMSI、ports、namespace、log 都不同。
- [x] Open5GS 有三笔正确 subscriber。
- [x] gNB 同时连接 Open5GS 与 FlexRIC。
- [x] Broker 能建立 gNB 与三台 UE 的 ZMQ connections。
- [x] UE1、UE2、UE3 都完成 Random Access。
- [x] UE1、UE2、UE3 都进入 RRC Connected。
- [x] UE1、UE2、UE3 都建立 PDU Session 并取得不同 IP。
- [x] 三个 namespace 都有 `tun_srsue`。
- [x] 三台 UE 都能 ping `10.45.0.1`。
- [x] 三台 UE 都能分别通过 iperf3 产生流量。
- [x] gNB 或 KPM log 能看到多个 UE/RNTI。
...
- [x] Broker 可用无 GUI Python process 启动（`QT_QPA_PLATFORM=offscreen`）。
